...
if (emailForm) {
  emailForm.addEventListener('submit', function(e) {
    ...
    const emailInput = document.getElementById('gatewayEmailInput').value;
    
    if(emailInput) {
      // 1. Show the loading spinner overlay
      const overlay = document.getElementById('calculatingOverlay');
      if (overlay) overlay.style.display = 'flex'; 
      
      // 2. Wait 1.5 seconds, then hide spinner and show results
      setTimeout(() => {
        ...
        // Trigger the math calculations to build the chart and score
        if (typeof calculateAndRenderResults === 'function') {
